...
class MDMModel:
    def __init__(
        ...
        with tf.name_scope('Network', values=[self.in_init_shapes]):
            ...
            self.rnn_cell = tf.zeros((self.batch_size, self.hidden_size), name='cell_state')
            ...
            for step in range(self.num_iterations):
                ...
                with tf.variable_scope('rnn', reuse=tf.AUTO_REUSE, auxiliary_name_scope=False):
                    with tf.name_scope('mdm_rnn{}'.format(step), values=[rnn_in, self.rnn_hidden, self.rnn_cell]):
                        x = tf.concat([rnn_in, self.rnn_hidden], 1)
                        f = tf.layers.dense(
                            x, self.hidden_size,
                        ...
                        self.rnn_hidden = o * tf.tanh(self.rnn_cell)
                        prediction = tf.layers.dense(
                            o, self.num_patches * 2,
