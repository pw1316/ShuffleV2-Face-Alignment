...
def load_opencv_detector():
    ...
    return partial(opencv_detector, greyscale=False)
...
def load_pico_detector():
    ...
    return partial(pico_detector, greyscale=False)
...
def load_n_create_generator(pattern, detector_name, group=None, overwrite=False):
    # from menpo.landmark import LandmarkGroup
    from menpo.model import PCAModel
    try:
    ...
    bboxes = [(img, detect_and_check(img, cur_detector, group=group))
