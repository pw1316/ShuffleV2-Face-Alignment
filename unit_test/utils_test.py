...
assert isinstance(test_image_m, mimage.Image)

# Mirror bb
plt.subplot(211)
test_image.landmarks['bb_test'] = mshape.PointCloud(test_image.landmarks['bb'].points[[0, 1]])
test_image.view_landmarks(group='bb_test')
plt.subplot(212)
test_image_m.landmarks['bb_test'] = mshape.PointCloud(test_image_m.landmarks['bb'].points[[0, 1]])
test_image_m.view_landmarks(group='bb_test')
plt.show()

# Mirror 68
plt.subplot(211)
test_image.landmarks['PTS_test'] = mshape.PointCloud(test_image.landmarks['PTS'].points[range(10)])
test_image.view_landmarks(group='PTS_test')
plt.subplot(212)
test_image_m.landmarks['PTS_test'] = mshape.PointCloud(test_image_m.landmarks['PTS'].points[range(10)])
...
plot_images = batch_draw_landmarks(np_images, np_lms, np_lms)
plot_images = [
